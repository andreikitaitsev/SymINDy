...
class SymINDy_class(object):
    ...
    def evalSymbReg(
        ...
        def fit_sindy_model(model, x_train, x_dot_train=None):
            '''Fit pysindy model.
            Inputs:                
            '''
        ...
        def score_sindy_model(x_train, time_rec_obs, x_dot_train,
        ...
        if tr_te_ratio is not None:
            ...
            x_train_te = x_train[int(tr_te_ratio*len(x_train)):]
            if x_dot_train is not None:
                x_dot_train_tr = x_dot_train[:int(0.8* len(x_train))]
                x_dot_train_te = x_dot_train[int(0.8* len(x_train)):]
